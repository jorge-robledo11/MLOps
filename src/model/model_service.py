...
class ModelService:
    ...
    def load_model(self):
        ...
        if not model_path.exists():
            logger.warning(f'Modelo no encontrado en {model_path}. Construyendo el modelo...')
            build_model()  # Construye el modelo
    ...
    def predict(self, features: list) -> np.ndarray:
        ...
        """
        features_df = pd.DataFrame([features], columns=features_settings.features_prediction)
